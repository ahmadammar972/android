...
public class CharacteristicOperationExampleFullLeadsChestActivity extends AppCompatActivity implements OnChartValueSelectedListener {
    ...
    final String publishTopic = "tb/mqtt-integration/sensors/ecg/SN-001/data/LeadsChest";
    ...
    public static Intent startActivityIntent(Context context, String peripheralMacAddress, UUID characteristicUuid) {
        Intent intent = new Intent(context, CharacteristicOperationExampleFullLeadsChestActivity.class);
    ...
    private void onNotificationReceived(byte[] bytes) {
        ...
        if(characteristicUuid.compareTo(FULL_ECG_STREAM_DATA) == 0) {
            ...
            if (set == null || set1 == null || set2 == null || set3 == null || set4 == null || set5 == null) {
                set = createSet("Lead V1");
                data.addDataSet(set);

                set1 = createSet("Lead V2");
                data1.addDataSet(set1);

                set2 = createSet("Lead V3");
                data2.addDataSet(set2);

                set3 = createSet("Lead V4");
                data3.addDataSet(set3);

                set4 = createSet("Lead V5");
                data4.addDataSet(set4);

                set5 = createSet("Lead V6");
                data5.addDataSet(set5);
            }
            data.addEntry(new Entry(set.getEntryCount(), values[7]), 0); // lead V1
            data1.addEntry(new Entry(set1.getEntryCount(), values[3]), 0); // lead V2
            data2.addEntry(new Entry(set2.getEntryCount(), values[4]), 0); // lead V3
            data3.addEntry(new Entry(set3.getEntryCount(), values[5]), 0); // lead V4
            data4.addEntry(new Entry(set4.getEntryCount(), values[6] ), 0); // lead V5
            data5.addEntry(new Entry(set5.getEntryCount(), values[0]), 0); // lead V6
...
//                .trim();
    ...
    class ScheduledExecutor extends AbstractScheduledService {
        ...
        protected Scheduler scheduler() {
            ...
            return Scheduler.newFixedRateSchedule(0, 4, TimeUnit.SECONDS);
        }
